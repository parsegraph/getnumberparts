...
app.get('/', async (req, res) => {
  ...
  write(`<html>`);
  write(`<head>`);
  write(`<title>getnumberparts</title>`);
  write(`</head>`);
  write(`<body>`);
  write(`<h1>getnumberparts <a href='/coverage'>Coverage</a> <a href='/docs'>Docs</a></h1>`);
  write(`<p>This library is available as JavaScript UMD module: <a href='/parsegraph-getnumberparts.js'>parsegraph-getnumberparts.js</a></p>`);
...
app.listen(port, () => {
  console.log(`See getnumberparts build information at http://localhost:${port}`);
});
